...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        """
        Evaluate a mathematical expression using basic operators only.
        ...
        try:
            if not expression or not expression.strip():
                msg = "Calculator error: expression is empty."
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg

            expr = expression.strip()

            # Safety validation: only basic math characters/operators
            allowed_pattern = r"^[0-9\.\+\-\*\/%\(\)\s]+$"
            if not re.fullmatch(allowed_pattern, expr):
                msg = "Calculator error: expression contains invalid characters."
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg
            ...
            if any(token in expr for token in forbidden_tokens):
                msg = "Calculator error: unsafe expression detected."
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg
            ...
            msg = "Calculator error: division by zero."
            logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
            return msg
        except Exception as e:
            msg = f"Calculator error: {str(e)}"
            logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
            return msg
