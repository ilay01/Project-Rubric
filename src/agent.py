...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    user_intent = llm_with_structure.invoke(prompt)
    intent = user_intent.intent_type
    
    # Extract intent and set next_step
...
def create_workflow(llm, tools):
    ...
    #LEARNING this is also a node to be added 
    ...
    # LEARNING auch hier kommen die node names und nicht die functions
